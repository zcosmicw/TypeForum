      ...
      <div className="border-b border-white/10 surface-panel">
        <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
          ...
          <div className="flex items-start gap-4">
            <span className="flex h-14 w-14 items-center justify-center rounded-xl bg-slate-800 text-2xl text-brand-teal">
              {category.icon}
            </span>
            <div>
          ...
          <div className="mt-6 flex flex-wrap gap-2.5">
            <Link
              ...
              className={`rounded-full px-3.5 py-1 text-xs font-semibold transition-all duration-150 ${
                !sub
                  ? "bg-gradient-to-r from-teal-500 to-cyan-400 text-white shadow-[0_0_10px_rgba(45,212,191,0.2)]"
      ...
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
          ...
          <Link
            ...
            className="btn-main rounded-lg px-4.5 py-2 text-sm font-semibold text-white shadow-md transition-all"
          >
            New thread
          </Link>
        </div>
        ...
        {categoryThreads.length > 0 ? (
          <div className="panel-border overflow-hidden rounded-xl border border-white/10 surface-panel">
          ...
          <div className="rounded-xl border border-dashed border-white/10 bg-slate-800/30 px-6 py-12 text-center">
            ...
            <Link
              href={`/c/${slug}/new${sub ? `?sub=${sub}` : ""}`}
              className="mt-4 inline-block rounded-lg bg-gradient-to-r from-teal-600 to-teal-400 px-4 py-2 text-sm font-semibold text-white hover:scale-[1.02] hover:shadow-[0_0_12px_rgba(45,212,191,0.3)]"
            >
              Create first thread
            </Link>
          </div>
